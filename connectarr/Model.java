...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int c = 0; c < _width; c++) {
            for (int r = 0; r < _height; r++) {
                int seqnum = _solution [c][r];
                if (seqnum == 1) {
                    Place np = solnNumToPlace(seqnum + 1);
                    _board [c][r] = new Sq(c, r, 1, true,
                                    dirOf(c, r, np.x, np.y), 0);

                } else if (seqnum == _width * _height) {
                    _board [c][r] =
                            new Sq(c, r, width() * height(), true, 0, 0);
                } else {
                    Place np = solnNumToPlace(seqnum + 1);
                    _board [c][r] = new Sq(c, r, 0, false,
                                    dirOf(c, r, np.x, np.y), -1);
                }
                _board[c][r]._predecessors = new PlaceList();
                _allSquares.add(_board[c][r]);
            }
        }
        for (int test = 1; test < _solnNumToPlace.length; test++) {
        ...
        for (Sq[] cols: _board) {
            for (Sq element: cols) {
                element._successors =
                        _allSuccessors[element.x][element.y][element._dir];
                for (Place pred : element._successors) {
                    get(pred)._predecessors.add(element.pl);
                }
            }
        }
        _unconnected = last - 1;
    ...
    Model(Model model) {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                ...
                _board[x][y]._successors = model._board[x][y]._successors;
                _board[x][y]._predecessors = model._board[x][y]._predecessors;
    ...
    boolean autoconnect() {
        ...
        for (Sq first : _allSquares) {
            for (Sq second :_allSquares) {
                if (first._sequenceNum != 0 && second._sequenceNum != 0) {
                    if (first.connectable(second)
                            && first._sequenceNum == second._sequenceNum - 1) {
    ...
    private int arrowDirection(int x, int y) {
        ...
        if (_board[x][y]._successor == null) {
            ...
            for (int col = 0; col < _solution.length; col++) {
                for (int row = 0; row < _solution[0].length; row++) {
                    if (_solution[col][row] == seq1) {
                        return Place.dirOf(x, y, col, row);
                    }
                }
            }
        }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            ...
            if (_sequenceNum == _height * _width) {
                return false;
            }
            PlaceList possuccessor = _allSuccessors[x][y][_dir];

            if (possuccessor.contains(ps1)) {
        ...
        boolean connect(Sq s1) {
            ...
            int groupbefore = _group;
            int s1before = s1.sequenceNum();
            int mebefore = sequenceNum();
            if (_hasFixedNum && s1.sequenceNum() == 0
                    || sequenceNum() != 0 && s1.sequenceNum() == 0) {
                int numnext = _sequenceNum + 1;
                for (Sq n = successor(); n != null; n = n.successor()) {
                    n._head = _head; n._sequenceNum = numnext;
                    numnext += 1;
                }
            } else if (s1._hasFixedNum && sequenceNum() == 0
                    || s1.sequenceNum() != 0 && sequenceNum() == 0) {
                int numlast = s1.sequenceNum() - 1;
                for (Sq last = this; last != null; last = last._predecessor) {
                    last._sequenceNum = numlast;
                    numlast -= 1;
                }
                for (Sq c = _successor; c != null; c = c._successor) {
                    c._head = c;
                }
            }
            if (mebefore == 0 && s1before == 0) {
                if (_group != -1 && s1._group == -1) {
                    s1._head = _head; s1._group = _group;
                } else if (_group == -1 && s1._group != -1) {
                    _group = s1.group();
                    for (Sq ch = s1; ch != null; ch = ch._successor) {
                        ch._head = _head;
                    }
                } else if (_group != -1 && s1._group != -1) {
                    _head._group = joinGroups(s1._group, _group);
                    for (Sq c = _head._successor; c != null; c = c._successor) {
                        c._head = _head; c._group = _head._group;
                    }
                } else {
                    s1._head = _head; _group = s1._group = newGroup();
                }
            }
            if ((s1before == 0 && s1.sequenceNum() != 0)
                    || (mebefore == 0 && sequenceNum() != 0)) {
                if (s1before == 0 && s1.sequenceNum() != 0) {
                    releaseGroup(sgroup);
                } else if (mebefore == 0 && sequenceNum() != 0) {
                    releaseGroup(groupbefore);
                }
            }
            return true;
        }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (_predecessor == null && next._successor == null) {
                    releaseGroup(_group); releaseGroup(next.group());
                    _group = next._group = -1;
                } else if (_predecessor == null || next._successor == null) {
                    if (_predecessor == null) {
                        _group = -1;
                        for (Sq ch = next; ch != null; ch = ch._successor) {
                            ch._head = next;
                        }
                    } else {
                ...
                boolean fixnumprev = false; boolean fixnumnext = false;
                for (Sq s = this; s != null; s = s._predecessor) {
                    if (get(s).hasFixedNum()) {
                        fixnumprev = true;
                    }
                }
                if (!fixnumprev) {
                    if (_predecessor != null) {
                        _group = newGroup();
                        for (Sq s = this; s != null; s = s._predecessor) {
                            s._sequenceNum = 0; s._group = _group;
                        }
                    } else {
                        _group = -1;
                    }
                ...
                if (!fixnumnext) {
                    ...
                    for (Sq s = next; s != null; s = s._successor) {
                        s._sequenceNum = 0; s._group = next._group;
                    }
                } else {
                    next._group = -1;
                }
