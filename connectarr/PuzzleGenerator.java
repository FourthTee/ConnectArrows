            ...
            if (sq.successor() == null && sq.direction() != 0) {
                for (Place place: sq.successors()) {
                    if (sq.connectable(model.get(place))) {
                        found = model.get(place);
                        nFound += 1;
                    }
                }
                if (sq.sequenceNum() != 0) {
                    for (Place place: sq.successors()) {
                        Sq targetsq = model.get(place);
                        if (targetsq.sequenceNum() != 0) {
                            if (sq.connectable(targetsq)) {
                                found = targetsq;
                                nFound = 1;
                            }
                        }
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (Place place: sq.predecessors()) {
                    Sq predsq = model.get(place);
                    if (predsq.connectable(sq)) {
                        found = predsq;
                        nFound += 1;
                    }
                }
                if (sq.sequenceNum() != 0) {
                    for (Place place: sq.successors()) {
                        Sq predsqnum = model.get(place);
                        if (predsqnum.sequenceNum() != 0) {
                            if (predsqnum.connectable(sq)) {
                                found = predsqnum;
                                nFound = 1;
                            }
                        }
